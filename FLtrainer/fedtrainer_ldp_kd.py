...
class Trainer(AbstractTrainer):
    ...
    def _train_epoch_A(self, train_data, epoch_idx, global_embedding, loss_func=None, show_progress=False):
        ...
        for batch_idx, interaction in enumerate(iter_data):
            ...
            losses = self.model_A.calculate_loss(interaction, global_embedding) #+ 0.005 * torch.norm(self.model_A.pq_code_embedding.weight - global_embedding.weight) ** 2
    ...
    def _train_epoch_B(self, train_data, epoch_idx, global_embedding, loss_func=None, show_progress=False):
        ...
        for batch_idx, interaction in enumerate(iter_data):
            ...
            losses = self.model_B.calculate_loss(interaction, global_embedding) #+ 0.005 * torch.norm(self.model_B.pq_code_embedding.weight - global_embedding.weight) ** 2
            if isinstance(losses, tuple):
                loss = sum(losses)
                loss_tuple = tuple(per_loss.item() for per_loss in losses)
                total_loss = loss_tuple if total_loss is None else tuple(map(sum, zip(total_loss, loss_tuple)))
