...
class Trainer(AbstractTrainer):
    ...
    def __init__(self, config_A, config_B, model_A, model_B):
        ...
        self.batch_size = config_A['train_batch_size']
    ...
    def _train_epoch_A(self, train_data, epoch_idx, global_embedding, loss_func=None, show_progress=False):
        ...
        self.model_A.train()
        model_B_pq_codes = self.model_B.pq_codes

        #抽取
        N   = model_B_pq_codes.size(0)
        idx = torch.randint(0, N, (self.batch_size,), device=model_B_pq_codes.device)
        sampled_codes = model_B_pq_codes[idx] 
        ...
        for batch_idx, interaction in enumerate(iter_data):
            ...
            losses = self.model_A.calculate_loss(interaction, sampled_codes) #+ 0.005 * torch.norm(self.model_A.pq_code_embedding.weight - global_embedding.weight) ** 2
    ...
    def _train_epoch_B(self, train_data, epoch_idx, global_embedding, loss_func=None, show_progress=False):
        self.model_B.train()
        model_A_pq_codes = self.model_A.pq_codes
        # 随机抽取 self.batch_size 行，作为负样本 codes
        N = model_A_pq_codes.size(0)
        idx = torch.randint(0, N, (self.batch_size,), device=model_A_pq_codes.device)
        sampled_codes = model_A_pq_codes[idx]
        ...
        for batch_idx, interaction in enumerate(iter_data):
            ...
            losses = self.model_B.calculate_loss(interaction, sampled_codes)  # 对称于 _train_epoch_A
            if isinstance(losses, tuple):
                loss = sum(losses)
                loss_tuple = tuple(per_loss.item() for per_loss in losses)
                total_loss = loss_tuple if total_loss is None else tuple(map(sum, zip(total_loss, loss_tuple)))
        ...
        for epoch_idx in range(self.start_epoch, self.pretrain_epochs):
            ...
            if (epoch_idx + 1) % self.save_step == 0:
                saved_model_file_A = os.path.join(
                    self.checkpoint_dir,
                    '{}-{}-{}-{}-t{}-w{}.pth'.format(self.config_A['model'], self.config_A['dataset'], str(epoch_idx + 1), str(date.today()), 
                                                     self.config_A['contrastive_temperature'], self.config_A['contrastive_loss_weight_ss'])
                )
                saved_model_file_B = os.path.join(
                    self.checkpoint_dir,
                    '{}-{}-{}-{}-t{}-w{}.pth'.format(self.config_B['model'], self.config_B['dataset'], str(epoch_idx + 1), str(date.today()), 
                                                     self.config_B['contrastive_temperature'], self.config_B['contrastive_loss_weight_ss'])
                )
