...
if __name__ == '__main__':
    ...
    for did, ds in enumerate(dataset_names):
        npy_path = os.path.join(args.input_path, short_name, f'{ds}.{args.suffix}')
        map_path = os.path.join(args.input_path, short_name, f'{ds}.{args.suffix1}')
        vecs = np.load(npy_path, mmap_mode="r" if True else None)  # 去掉 mmap_mode
        # 如果是对象数组，再转成数值矩阵
        if vecs.dtype == object:
            vecs = np.array([np.asarray(x, dtype=np.float32) for x in vecs])
        
        assert vecs.shape[1] == args.plm_size
        print(f"Loaded seq2bert bank from {npy_path} with shape {vecs.shape}.")
        key2row = {}
        ...
        if args.strict:
            for uid in filter_user_id_list[did].tolist():
                ...
                this_user_seq_index = key2row.get(this_user_items_seq, -1)
                if this_user_seq_index == -1:
                    print('Error! user id: ', uid, 'items: ', this_user_items_seq, 'not in the seq2bert bank!')
                    raise ValueError('Invalid user sequence index')
                this_user_emb = vecs[this_user_seq_index]
    ...
    save_index_path = os.path.join(
        args.output_path,
        short_name,
        f"{short_name}_user_LLMEasy.OPQ{args.subvector_num},IVF1,PQ{args.subvector_num}x{args.n_centroid}{'.strict' if args.strict else ''}.index")
