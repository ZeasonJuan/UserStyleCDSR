...
if __name__ == '__main__':
    ...
    for kdw in [0.1, 0.2, 0.5, 1]: 
        parser = argparse.ArgumentParser()
        parser.add_argument('-d', type=str, default='OA', help='dataset name')
        parser.add_argument('-regularization_loss_weight', type=float, default=kdw, help='weight for regularization loss')
        args, _ = parser.parse_known_args()
        # Call pretrain with these hyperparameters
        model, dataset = pretrain(args.d, regularization_loss_weight=args.regularization_loss_weight)
