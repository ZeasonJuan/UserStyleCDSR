...
class VQRecCL(SequentialRecommender):
    def __init__(self, config, dataset):
        ...
        self.contrastive_loss_weight_ss = config['contrastive_loss_weight_ss']
        self.contrastive_loss_weight_sv = config['contrastive_loss_weight_sv']
    ...
    def contrastive_loss(self, seq_output, pos_items_emb, temperature=0.1, pq_codes_embeddings_from_other_domain=None):
        """
        ...
        if pq_codes_embeddings_from_other_domain is not None:
            # -------- (1) s ↔ s_aug & s ↔ v 对比  --------
            # positive: s with v ; negative: s with v_other
            v_other = F.normalize(pq_codes_embeddings_from_other_domain, dim=-1)
            logits_pos = (seq_output * pos_items_emb).sum(dim=-1, keepdim=True) / temperature   # [B,1]
            logits_neg = (seq_output * v_other).sum(dim=-1, keepdim=True) / temperature          # [B,1]
            logits = torch.cat([logits_pos, logits_neg], dim=1)                                  # [B,2]
            labels = torch.zeros(seq_output.size(0), dtype=torch.long, device=seq_output.device) # 0 是正样本列
            loss = F.cross_entropy(logits, labels)
            return loss * self.contrastive_loss_weight_sv
        ...
        return loss_ss * self.contrastive_loss_weight_ss + loss_sv * self.contrastive_loss_weight_sv
    ...
    def calculate_loss(self, interaction, pq_codes_from_other_domain=None):
        ...
        pos_items_emb = self.pq_code_embedding(pos_items_code).mean(dim=-2)  # [B H]
        if pq_codes_from_other_domain is not None:
            assert item_seq.shape[0] == pq_codes_from_other_domain.shape[0]
            pq_codes_embeddings_from_other_domain = self.pq_code_embedding(pq_codes_from_other_domain).mean(dim=-2)  # [B H]
            contrastive_loss = self.contrastive_loss(seq_output, pos_items_emb, self.contrastive_temperature, pq_codes_embeddings_from_other_domain)
        else:
            contrastive_loss = self.contrastive_loss(seq_output, pos_items_emb, self.contrastive_temperature)
        if self.loss_type == 'BPR':
            raise NotImplementedError()
