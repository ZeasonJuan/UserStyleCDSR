...
class id_prompt_KD_no_both(SequentialRecommender):
    def __init__(self, config, dataset, vqrecKD):
    ...
    def forward(self, item_seq, item_seq_len):
        ...
        position_embedding = self.vqrecKD.position_embedding(position_ids)
        item_emb = self.item_embedding(item_seq)
        item_emb = item_emb + position_embedding
        item_emb = self.LayerNorm(item_emb)
        item_trm_input = self.dropout(item_emb)
        extended_attention_mask = self.vqrecKD.get_attention_mask(item_seq)
        ...
        feature_output = self.vqrecKD.forward(item_seq, item_seq_len)

        output = feature_output * item_output  # [B H]
    ...
    def calculate_item_emb(self):
        pq_code_emb = self.vqrecKD.pq_code_embedding_specific(self.pq_codes).mean(dim=-2)
        return pq_code_emb
...
class id_prompt_KD_no_prompt(SequentialRecommender):
    def __init__(self, config, dataset, vqrecKD):
        super().__init__(config, dataset)
        self.pq_codes = dataset.pq_codes
        self.index_assignment_flag = False
...
class id_prompt_KD(SequentialRecommender):
    def __init__(self, config, dataset, vqrecKD):
        super().__init__(config, dataset)
        self.pq_codes = dataset.pq_codes
        self.index_assignment_flag = False
